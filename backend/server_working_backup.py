...
# Warm-up pass on startup so MPS kernels are compiled and weights are in memory
# before the server begins accepting client requests.
print(f"Warming up Pellav2 detector on {DEVICE}...")
_warm_t0 = time.perf_counter()
with torch.inference_mode():
    _dummy = torch.zeros(1, CROP_SAMPLES, device=DEVICE)
    _ = MODEL(_dummy)
    if DEVICE == "mps":
        torch.mps.synchronize()
print(f"Pellav2 detector warmed up and ready in {(time.perf_counter() - _warm_t0):.2f}s.")
...
def detect(audio_bytes):
    ...
    preprocessing_ms = (time.perf_counter() - t_pre_start) * 1000
