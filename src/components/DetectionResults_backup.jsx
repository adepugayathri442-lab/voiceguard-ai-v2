...
export default function DetectionResults({ 
  ...
  const isClone = result.isCloneAlert || result.state === 'STATE_3_AI_CLONE_ATTACK' || result.state === 'STATE_3_AI_VOICE_CLONE';
  const isGenuine = result.state === 'STATE_1_GENUINE_USER';
  const isGenericAi = result.state === 'STATE_2_GENERIC_AI_VOICE';
  const isUnknown = result.state === 'STATE_4_UNKNOWN_HUMAN';

  const riskScore = result.riskScore ?? (isClone ? 96 : isGenericAi ? 72 : isUnknown ? 36 : 12);
  const riskLevel = result.riskLevel || (riskScore >= 76 ? 'CRITICAL' : riskScore >= 51 ? 'HIGH' : riskScore >= 21 ? 'MODERATE' : 'LOW');
    ...
    <div className="w-full space-y-6 animate-fadeIn">
          ...
          <div>
            <div className="flex items-center space-x-2">
              <span className="font-mono font-bold text-sm text-white">
                {result.callerNumber || '+91 98765 43210'}
              </span>
              {result.knownCallerInfo?.isKnown ? (
                <span className="px-2 py-0.5 rounded text-[10px] font-mono font-bold bg-amber-500/15 text-amber-400 border border-amber-500/30 flex items-center space-x-1">
                  ...
                  <span>KNOWN CALLER ({result.knownCallerInfo.previousCallCount} past calls)</span>
                </span>
              ) : (
            ...
            <p className="text-[11px] text-slate-400 mt-0.5">
              Call ID: <code className="text-cyan-400">{result.callId || result.id}</code> • Duration: {result.durationSeconds || 5}s
            </p>
          </div>
        </div>
          ...
          <div className="flex items-start space-x-4">
            ...
            <div className="flex-1 space-y-2">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-xs font-mono text-rose-300">INCIDENT ID: #{result.id.slice(-6)}</span>
              </div>
              ...
              <p className="text-xs sm:text-sm text-rose-200/90 leading-relaxed font-medium">
                This voice strongly resembles the registered user ({result.voiceSimilarity}% biometric match) but shows synthetic/AI-generated characteristics. Do not approve any transactions or share sensitive info based on this call.
              </p>
          ...
          <div className="flex items-start space-x-3.5">
            ...
            <div className="flex-1 space-y-1.5">
              ...
              <p className="text-xs sm:text-sm text-amber-200/90 leading-relaxed">
                Acoustic vocoder phase discontinuities detected with {result.confidence}% confidence. Caller is non-biological automated speech. Independent verification required before sharing sensitive data.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-300">
                Biometric voice parameters match enrolled reference profile ({result.voiceSimilarity}% similarity).
              </p>
            </div>
          </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <div className="text-xs text-slate-300 font-mono">
              Identity Decision: <strong className="text-white">{result.identityResult}</strong>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className={`rounded-2xl p-6 relative transition-all duration-300 ${
          ...
          <div className="space-y-2.5 text-xs">
            ...
            <div className={`p-3 rounded-lg border flex items-start space-x-3 ${
              result.breakdown?.acousticPattern?.flagged ? 'bg-rose-950/20 border-rose-500/30 text-rose-200' : 'bg-slate-900/50 border-slate-800 text-slate-300'
            }`}>
              <div className={`mt-0.5 p-1 rounded-full ${
                result.breakdown?.acousticPattern?.flagged ? 'bg-rose-500/20 text-rose-400' : 'bg-emerald-500/20 text-emerald-400'
              }`}>
                {result.breakdown?.acousticPattern?.flagged ? <XCircle className="w-3.5 h-3.5" /> : <CheckCircle2 className="w-3.5 h-3.5" />}
              </div>
              <div>
                <span className="font-semibold block text-slate-100">
                  Acoustic pattern: {result.breakdown?.acousticPattern?.status}
                </span>
                <p className="text-[11px] text-slate-400 mt-0.5">
                  {result.breakdown?.acousticPattern?.detail}
                </p>
              </div>
            </div>

            <div className={`p-3 rounded-lg border flex items-start space-x-3 ${
              result.breakdown?.prosody?.flagged ? 'bg-rose-950/20 border-rose-500/30 text-rose-200' : 'bg-slate-900/50 border-slate-800 text-slate-300'
            }`}>
              <div className={`mt-0.5 p-1 rounded-full ${
                result.breakdown?.prosody?.flagged ? 'bg-rose-500/20 text-rose-400' : 'bg-emerald-500/20 text-emerald-400'
              }`}>
                {result.breakdown?.prosody?.flagged ? <XCircle className="w-3.5 h-3.5" /> : <CheckCircle2 className="w-3.5 h-3.5" />}
              </div>
              <div>
                <span className="font-semibold block text-slate-100">
                ...
                <p className="text-[11px] text-slate-400 mt-0.5">
                  {result.breakdown?.prosody?.detail}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className={`rounded-2xl p-6 relative transition-all duration-300 ${
          ...
          {!(result.cloneResult?.hasProfile || isClone || isGenuine || isGenericAi) ? (
                ...
                <div className="mt-4 pt-4 border-t border-slate-800/80">
                  ...
                  <div className="w-full h-2 rounded-full bg-slate-800 overflow-hidden">
                    <div 
                      ...
                      style={{ width: `${result.voiceSimilarity}%` }}
                    ></div>
                  </div>
                </div>
              </div>
                ...
                <div className="flex items-start space-x-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-slate-300 text-[11.5px]">
                      {result.cloneResult?.description}
                    </p>
                  </div>
                </div>
              </div>
      ...
      <div className="p-5 rounded-2xl glass-card space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
          ...
          <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800 space-y-1">
            <div className="flex items-center space-x-2">
              {result.forensicEvidence?.voiceSimilarityEvidence?.matched ? (
            ...
            <p className="text-slate-400 text-[11px] pl-6">
              {result.forensicEvidence?.voiceSimilarityEvidence?.status}
            </p>
          </div>
          ...
          <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800 space-y-1">
            <div className="flex items-center space-x-2">
              ...
              <span className={`font-bold ${isAi ? 'text-rose-300' : 'text-slate-200'}`}>
                {result.forensicEvidence?.syntheticEvidence?.status}
              </span>
            </div>
            <p className="text-slate-400 text-[11px] pl-6">
              {result.forensicEvidence?.syntheticEvidence?.detail}
            </p>
          </div>
          ...
          <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800 space-y-1">
            <div className="flex items-center space-x-2">
              ...
              <span className={`font-bold ${isAi ? 'text-rose-300' : 'text-slate-200'}`}>
                {result.forensicEvidence?.prosodyEvidence?.status}
              </span>
            </div>
            <p className="text-slate-400 text-[11px] pl-6">
              {result.forensicEvidence?.prosodyEvidence?.detail}
            </p>
          </div>
          ...
          <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800 space-y-1">
            <div className="flex items-center space-x-2">
              ...
              <span className={`font-bold ${!isAi ? 'text-emerald-300' : 'text-rose-300'}`}>
                {result.forensicEvidence?.antiSpoofEvidence?.status}
              </span>
            </div>
            <p className="text-slate-400 text-[11px] pl-6">
              Score: {result.antiSpoofScore || 85}/100 physical acoustic authenticity.
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="p-4 rounded-xl bg-slate-950/80 border border-cyan-500/40 space-y-3 animate-fadeIn">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              <div className="text-xs">
                ...
                <span className="text-slate-400 text-[11px]">Security code dispatched to {result.callerNumber}: <code className="text-cyan-300 font-mono font-bold">749210</code></span>
              </div>
              <button
                type="button"
          ...
          {playableUrl ? (
            <>
              ...
              <span className="text-[11px] font-mono text-slate-400 hidden sm:inline">
                Duration: {result.durationSeconds || '5'}s
              </span>
            </>
          ) : (
